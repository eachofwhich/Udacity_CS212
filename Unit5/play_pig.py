...
def play_pig(A, B):
    ...
    while True:
        turn, me, you, pending = current_state

        if goal == me: return A
        elif goal == you: return B

        next_move = strategies[turn](current_state)

        die = random.randint(1, 6)
        current_state = roll(current_state, die) if 'roll' == next_move else hold(current_state)

    return strategies[current_state[0]]
