...
def Pwin3(me, you, pending):
    """Return the utility of the current state by backtracking from the end states."""

    if me + pending >= goal: return 1
    if you >= goal: return 0

    hold = 1 - Pwin3(you, me + pending, 0) if pending else 0

    roll = ( 1 - Pwin3(you, me + 1, 0) + sum( Pwin3(me, you, pending + die) 
                                            for die in (2, 3, 4, 5, 6) ) ) / 6.0
...
def test():
    ...
    result = abs(Pwin2((0, 25, 32, 8)) - 0.736357188272)
    assert result <= epsilon, ('result', result)
