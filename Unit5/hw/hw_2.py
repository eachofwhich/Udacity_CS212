...
def pig_actions_d(state):
    ...
    result = []
    if double is 1: result.append('double')
    if pending: result.append('hold')
    result.append('roll')
...
def strategy_d(state):
    ...
    if me + pending >= goal and double is not 2:
        return 'double'
...
def best_action(state):
    ...
    if pending >= goal/2 or me + pending >= goal:
        return 'hold'

    def assess(action): return quality(state, action)
    return max(['roll', 'hold'], key=assess)
...
def quality(state, action):
    ...
    if 'hold' == state:
        return 1 - utility( do('hold', state, dierolls()) )

    if 'roll' == state:
        return (
                1 - utility( do('roll', state, iter([1])) ) +
                sum( utility( do('roll'), state, iter([die]) ) for die in (2, 3, 4, 5, 6) )
                ) / 6
...
def utility(state):
    ...
    if you >= goal: return 0

    return max( quality(state, action) for action in ['roll', 'hold'] )
