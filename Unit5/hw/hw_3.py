...
def quality(state, action):
    ...
    if 'wait' == action:
        possible_cards = []
        if 'F' in cards: possible_cards.append('F')
        if 'H' in cards: possible_cards.append('H')

        return sum(
                utility( do_sim(state, 'wait', card) ) for card in possible_cards
                ) / 2 
...
def utility(state):
    ...
    if 0 == len(cards): return score + yard

    return max(
                quality(state, action) for action in valid_actions(state)
                )
