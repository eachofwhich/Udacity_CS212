...
def find_prefixes(hand, pre = '', results = None, cache = True):
     """Find all prefixes (of words) that can be made from letters in hand."""
     if hand in PREFIX_CACHE: return PREFIX_CACHE[hand]
     ...
     if pre in PREFIXES:
         for L in hand:
             find_prefixes(hand.replace(L, '', 1), pre+L, results, False)

     if cache: PREFIX_CACHE[hand] = results
     
     return results



print find_prefixes(''.join(LETTERS), '')
# print PREFIX_CACHE