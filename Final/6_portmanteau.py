...
def get_suffixes(word):
    """
    Return a list of suffixes of word.

    The word itself is not included. Longest suffixes first.
    """

    result = [ word[index:] for index in xrange(1, len(word) ) ]
    return result



def get_prefixes(word):
    """Return prefixes of word. Word itself is not included."""
    return [ word[:index] for index in xrange(1, len(word)) ]



def try_words(word_one, word_two, get_score):
    """Return a set of portmanteaux between word_one and word_two."""

    word_one_suffixes = get_suffixes(word_one)
    word_two_prefixes = get_prefixes(word_two)

    for suffix in word_one_suffixes:
        for prefix in word_two_prefixes:
            assert(suffix)

            if suffix == prefix:

                mid = suffix
                start = word_one[:-len(mid)]
                end = word_two[len(mid):]

                score = get_score(start, mid, end)

                result = (start + mid + end, score)
                return set([result])
...
def get_score(start, mid, end):
    total_length = len(start + mid + end)
    start_length, mid_length, end_lengh = len(start), len(mid), len(end)

    ideal_start_length = ideal_end_length = total_length / 4

    ideal_mid_length = ideal_start_length * 2

    result = total_length - ( abs(start_length - ideal_start_length) )
    result -= abs(mid_length - ideal_mid_length)
    result -= abs(end_lengh - ideal_end_length)
...
def natalie(words):
    ...
    result = set() # tuple(portmanteau, score)


    for word_one in words:
        for word_two in words:
            if word_one == word_two: continue
            result = result | try_words(word_one, word_two, get_score)
...
def test_natalie():
    ...
    # test get_score()
    assert get_score('adole', 'scent', 'ed') == 8

    # test get_suffixes()
    print 'All good with get_suffixes()' if get_suffixes('abc') == ['bc', 'c'] else 'Failed on get_suffixes()'

    # test get_prefixes()
    print 'All good with get_prefixes()' if get_prefixes('abc') == ['a', 'ab'] else 'Failed on get_prefixes()'


    # test try_words()
    result = try_words('entrepreneur', 'neuropsychologist', get_score)
    print 'All good with try_words()' if result else 'Failed on try_words()'
