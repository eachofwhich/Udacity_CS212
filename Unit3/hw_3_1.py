...
def parse(start_symbol, text, grammar):
    ...
    def parse_atom(atom, text):
        if atom in grammar:  # Non-Terminal: tuple of alternatives
            ...
            debug('Found {} in grammar.'.format(atom))
            ...
            debug('{} must be an RE.'.format(atom))

            m = re.match(tokenizer % atom, text)

            if not m:
                debug( 'No match for "{}" in "{}"'.format(tokenizer % atom, text) )
            else:
                debug( 'Found a match for "{}" in "{}"'.format(tokenizer % atom, text) )
...
def debug(message, override = False):
...
def test():
    ...
    result = json_parse('["testing", 1, 2, 3]')
    debug(result, False)
    check = (                      
    ...
    result = json_parse('-123.456e+789')
    debug(result, False)
    check = ( 
                ['value', ['number', ['int', '-123'], ['frac', '.456'], 
                ['exp', 'e+789']]], '')
    
    assert  result == check

    result = json_parse('{"age": 21, "state":"CO","occupation":"rides the rodeo"}')
    debug(result, False)
    check = (
